...
# for Platt-scaling applied to gradient boosting:
from sklearnBoost import *
from PyML.containers import VectorDataSet
from PyML.classifiers.platt import *

# Model types:
# Platt-RBF not working (12-Jun-2017)
#MODELS = [LINEAR_SVM, RBF_SVM, RANDOM_FOREST, NAIVE_BAYES, DECISION_TREE, EXTRA_TREE, ADABOOST, GRADIENT_BOOST, PLATT_BOOST, PLATT_RBF] \
#       = ['linear', 'RBF', 'forest', 'bayes', 'dtree', 'extra', 'adaboost', 'gradient','Platt-boost','Platt-RBF']
#MODEL_CODES     = [LS_CODE, RS_CODE, RF_CODE, NB_CODE, DT_CODE, ET_CODE, AB_CODE, GB_CODE, PB_CODE, PR_CODE] = 'LRFNDEAGPT'
...
class ClassifierData(object) :
    ...
    def setData(self, labels, features) :
        ...
        if len(labels) != len(features) :
            raise ValueError('Number of labels %d does not match number of feature vectors %d' % (len(labels), len(features)))
        self.labels   = labels
...
def confusionMatrix(df, given, verbose=False, threshold=None) :
    ...
    if len(df) != len(given) :
        raise ValueError('Number of predictions must match number of training examples (%d != %d)' % (len(df), len(given)))
...
def dfFactory(model, data) :
    ...
    if type(model) in [SklearnBoost, Platt2] :
        X = VectorDataSet(data)
        results = model.test(X)
        return numpy.array(results.decisionFunc)
        ...
        ##return numpy.array([pair[1] for pair in class_probs])
        return result
...
def metadataToChromosomeMap(metaFile) :
    """Loads chromosome information from the given metadata file,
    where the metadata must have the form:
        chrom <TAB> pos <TAB> W <TAB> M <TAB> label
    """
    return ChromosomeMap(metaFile)
...
def removeChromosomes(classifierData, chromMap, chromosomeSet) :
    """Removes all data related to the given chromosomes from the classifier data
    and returns new ClassifierData and ChromosomeMap instances."""

    # None of the chromosomes to remove are in the map anyway
    if not (chromosomeSet & set(chromMap.keys())) :
        return (classifierData, chromMap)

    indexSet = set()
    for c in chromosomeSet :
        indexSet.update(chromMap[c][POS_LABEL])
        indexSet.update(chromMap[c][NEG_LABEL])

    labels   = []
    features = []
    metadata = []
    for i in range(len(classifierData)) :
        if i in indexSet : continue
        labels.append(classifierData.binary[i])
        features.append(classifierData.features[i])
        metadata.append(chromMap.metadata[i])

    return ClassifierData(features=features, labels=labels), ChromosomeMap(metadata)
...
#
# Need to write a multiprocessing version of this:
#
def runCV(model, data, labels, nfolds=5, **args) :
    ...
    # Recent version of sklearn completely breaks this!  Rather than
    # wading through their inadequate documentation to reconfigure
    # everything, and with the onerous possibility that it will change
    # again later, I wrote my own damned method.
...
def selectChromosomes(classifierData, chromMap, chromosomeSet) :
    """Selects only data related to the given chromosomes from the classifier data
    and returns new ClassifierData and ChromosomeMap instances."""
    # None of the chromosomes to include are in the map
    if not (chromosomeSet & set(chromMap.keys())) :
        return (None, None)
    ...
    filtered  = ClassifierData(features=features, labels=labels)
    refVector = classifierData.features[0]
    newVector = filtered.features[0]
    if len(refVector) != len(newVector) :
        raise ValueError('** error: filtered data has %d features; should have %d' % (len(newVector), len(refVector)))
    
    return filtered, ChromosomeMap(metadata)
...
def testModel(model, train, valid, pickleFile=None, weights=None, verbose=False) :
    """Performs a single train-validate cycle on a model."""
    if weights is None :
        if type(train.binary) == list :
            train.binary = numpy.array(train.binary)
        model.fit(train.features, train.binary)
    else :
        model.fit(train.features, train.binary, sample_weight=weights)

    if verbose :
        importance = model.feature_importances_
        sortedIndexes = numpy.argsort(importance)
        sys.stderr.write('Model importance values for %d features:\n' % len(importance))
        for i in sortedIndexes[::-1] :
            sys.stderr.write('%-3d\t%3d\t%.5g\n' % (i, (i+2), importance[i]))

    if pickleFile is not None :
        import pickle
        pickle.dump(model, open(pickleFile,'wb'))

    df     = dfFactory(model, valid.features)
    result = Results()
    result.add(1, df, valid.binary)
    return result
